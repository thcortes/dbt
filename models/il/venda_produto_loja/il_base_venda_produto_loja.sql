...
FROM
    ...
    LEFT JOIN dev.temp_Mediana_Loja_linha B ON A.Cnpj_loja = B.Cnpj_loja
    and A.id_linha = B.id_linha
    LEFT JOIN dev.temp_Mediana_Loja_Similar C ON A.Cnpj_loja = C.Cnpj_loja
    AND A.id_similar = C.id_similar
    LEFT JOIN dev.temp_venda_media_diaria_u30d D ON A.Cnpj_loja = D.Cnpj_loja
    AND A.sku = d.sku
    LEFT JOIN dev.temp_Subcat E ON A.Cnpj_loja = E.Cnpj_loja
    AND A.segmentos = E.segmentos
    AND A.categoria = E.categoria
    AND A.subcategoria = E.subcategoria
    LEFT JOIN dev.temp_Categoria F ON A.Cnpj_loja = F.Cnpj_loja
    AND A.segmentos = F.segmentos
    AND A.categoria = F.categoria
    LEFT JOIN dev.temp_segmentos G ON A.Cnpj_loja = G.Cnpj_loja
    AND A.segmentos = G.segmentos
    LEFT JOIN dev.temp_Moda H ON A.Cnpj_loja = H.Cnpj_loja
    and A.sku = H.sku;